...
class PDFConverterApp:
    def __init__(self, root):
        ...
        # Carregar a imagem
        self.bg_image = Image.open(r"C:\PyProjects\RemoveSign\background.png")
        ...
        # Labels
        ttk.Label(frame, text="Arquivo PDF:").grid(row=1, column=0, sticky=tk.W)

        # Btn        
        self.btn_selecionar = ttk.Button(frame, text="...", command=self.selecionar_pdf).grid(row=1, column=3, sticky=tk.W, padx=(0, 10))   
        self.lbl_selecionado = tk.Label(frame, text="(Selecione o arquivo)", wraplength=350)
        self.lbl_selecionado.grid(row=1, column=2, sticky=tk.W, padx=(0,10))

        # Botão
        self.btn_gerar = ttk.Button(frame, text="Gerar PDF", command=self.converter_pdf).grid(row=9, column=0, sticky=tk.W, padx=(0, 10), pady=10)        
